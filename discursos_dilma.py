...
for i in range(32):
	...
	df = pd.DataFrame()
	aux = i*30
	url = f"http://www.biblioteca.presidencia.gov.br/presidencia/ex-presidentes/dilma-rousseff/discursos/discursos?b_start:int={aux}"
	df = navigate(driver,url,df)
	base = pd.concat([base,df])

print(base)
base.to_csv("discursos_dilma.csv",sep="|",index=False)
